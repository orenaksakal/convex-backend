...
impl Exchange {
    ...
    /// as well as the ttl check interval if `inactive_connection_ttl` isn't
    /// `0`.
...
///
/// Actually `Pool` is a shared reference, i.e. every clone will lead to the
/// same instance created with [`Pool::new`]. Also `Pool` satisfies `Send` and
/// `Sync`, so you don't have to wrap it into an `Arc` or `Mutex`.
///
/// Note that you will probably want to await [`Pool::disconnect`] before
/// dropping the runtime, as otherwise you may end up with a number of
/// connections that are not cleanly terminated.
///
/// ## Multi-runtime environments
///
/// In multi-runtime environments such as actix-web, the pool's background tasks
/// must be explicitly started on the long-lived root runtime before worker
/// runtimes begin using the pool. See [`Pool::spawn_background_tasks`] for
/// details.
#[derive(Debug, Clone)]
pub struct Pool {
    opts: Opts,
...
impl Pool {
    ...
    /// Explicitly spawns the pool's background tasks on the current Tokio
    /// runtime.
    ///
    /// In most applications this method is unnecessary — the background tasks
    /// are spawned automatically on the first call to [`Pool::get_conn`].
    ///
    /// # When to use this
    ///
    /// In multi-runtime environments (e.g., [actix-web](https://actix.rs/)), the lazy
    /// spawning may cause the background tasks to be bound to a short-lived
    /// worker runtime rather than the application's long-lived root
    /// runtime. When that worker runtime shuts down, the [`Recycler`] is
    /// dropped, setting the pool's close flag and causing all subsequent
    /// [`Pool::get_conn`] calls to return [`DriverError::PoolDisconnected`].
    ///
    /// Call this method once during startup, from within the long-lived
    /// runtime, before any worker runtimes call [`Pool::get_conn`].
    ///
    /// **This method must be called from within the Tokio runtime that should
    /// own the background tasks.**
    ///
    /// # Idempotency
    ///
    ...
    /// Async function that disconnects this pool from the server and resolves
    /// to `()`.
    ///
    /// **Note:** This Future won't resolve until all active connections, taken
    /// from it, are dropped or disonnected. Also all pending and new
    /// `get_conn()`'s will resolve to error.
    ...
    fn send_to_recycler(&self, conn: Conn) {
        if let Err(conn) = self.drop.send(Some(conn)) {
            ...
            // This _probably_ means that the Runtime is shutting down, and that the
            // Recycler was dropped rather than allowed to exit cleanly.
    ...
    /// Removes an established in-use connection without returning it to the
    /// recycler.
    pub(super) fn discard_in_use_connection(&self) {
        self.inner
            .metrics
            .connections_in_use
            .fetch_sub(1, Ordering::Relaxed);
        let mut exchange = self.inner.exchange.lock().unwrap();
        exchange.exist = exchange
            .exist
            .checked_sub(1)
            .expect("discarded more connections than the pool owns");
        self.inner
            .metrics
            .connection_count
            .store(exchange.exist, Ordering::Relaxed);
        exchange.waiting.wake();
        let pool_is_closing = self.inner.close.load(Ordering::Acquire);
        drop(exchange);
        if pool_is_closing {
            // A direct discard bypasses the recycler channel. Wake the recycler
            // so concurrent pool shutdown can observe that the last connection
            // is gone.
            let _ = self.drop.send(None);
        }
    }

    #[cfg(test)]
    pub(super) fn register_in_use_connection_for_test(&self) {
        let mut exchange = self.inner.exchange.lock().unwrap();
        exchange.exist += 1;
        self.inner
            .metrics
    ...
    #[cfg(test)]
    pub(super) fn connection_counts_for_test(&self) -> (usize, usize, usize) {
        (
            self.inner
                .metrics
                .connections_in_use
                .load(Ordering::Relaxed),
            self.inner.exchange.lock().unwrap().exist,
            self.inner.metrics.create_failed.load(Ordering::Relaxed),
        )
    }

    /// Indicate that a connection failed to be created and release it.
    ///
    /// Decreases the exist counter since a broken or dropped connection should
    ...
    fn poll_new_conn(
        ...
        // otherwise the recycler may choose to exit, see that .exist == 0, and then
        // exit, and then we decide to create a new connection, which would then
        // never be torn down.
...
mod test {
    ...
    #[tokio::test]
    async fn force_disconnect_wakes_recycler_during_pool_shutdown() {
        let pool = Pool::new(crate::Opts::default());
        let (dropped, pool_opts) = pool.inner.exchange.lock().unwrap().recycler.take().unwrap();
        let mut recycler = Box::pin(Recycler::new(pool_opts, pool.inner.clone(), dropped));

        pool.register_in_use_connection_for_test();
        let mut conn = Conn::empty(crate::Opts::default());
        conn.inner.pool = Some(pool.clone());

        let mut disconnect = pin!(pool.clone().disconnect());
        assert!(matches!(poll!(disconnect.as_mut()), Poll::Pending));
        // Consume the initial shutdown marker and park on the recycler channel
        // while the artificial connection still counts as in use.
        assert!(matches!(poll!(recycler.as_mut()), Poll::Pending));

        conn.force_disconnect();

        timeout(Duration::from_secs(1), recycler.as_mut())
            .await
            .expect("recycler was not woken after force disconnect");
        // A production task drops its completed recycler future, which closes
        // the receiver observed by `Pool::disconnect`.
        drop(recycler);
        timeout(Duration::from_secs(1), disconnect)
            .await
            .expect("pool shutdown did not finish after force disconnect")
            .unwrap();
    }
    ...
    async fn should_check_inactive_connection_ttl() -> super::Result<()> {
        ...
        // check that connections are still in the pool because of
    ...
    async fn aa_should_hold_bounds2() -> super::Result<()> {
        ...
        while !conns.is_empty() {
            ...
            //  - if we drop one,  we still expect 10, because POOL_MIN limits the number of
            //    _idle_ connections (of which there is only 1)
            //  - once we've dropped 5, there are now 5 idle connections. thus, if we drop
            //    one more, we _now_ expect there to be only 9 connections total (no more
            //    connections should be pushed to idle).
    ...
    // Test that connections which err do not count towards the connection count in
    // the pool.
    #[tokio::test]
    ...
    async fn connection_in_use_metric() {
        ...
        // Ensure that we increase the connections in use metric when we get a
