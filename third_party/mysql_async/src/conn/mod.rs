...
static NEXT_LOCAL_GENERATION: AtomicU64 = AtomicU64::new(1);

/// Helper that asynchronously disconnects the givent connection on the default
/// tokio executor.
fn disconnect(mut conn: Conn) {
    ...
    if !disconnected {
        ...
        // this might fail if, say, the runtime is shutting down, but we've done what we
        // could
...
impl ConnInner {
    ...
    fn empty(opts: Opts) -> ConnInner {
        ...
        ConnInner {
            ...
            stream: None,
            local_generation: NEXT_LOCAL_GENERATION.fetch_add(1, Ordering::Relaxed),
            is_mariadb: false,
            version: (0, 0, 0),
            id: 0,
...
impl Conn {
    ...
    /// Returns a process-local identifier for this physical connection.
    ///
    /// Unlike the server connection identifier, this value is not reused when
    /// a later connection receives the same server identifier.
    pub fn local_generation(&self) -> u64 {
        self.inner.local_generation
    }
    ...
    /// Returns the ID generated by a query (usually `INSERT`) on a table with a
    /// column having the `AUTO_INCREMENT` attribute. Returns `None` if
    /// there was no previous query on the connection or if the query did
    /// not update an AUTO_INCREMENT value.
    ...
    /// Text information, as reported by the server in the last OK packet, or an
    /// empty string.
    ...
    /// Number of warnings, as reported by the server in the last OK packet, or
    /// `0`.
    ...
    /// [`crate::PoolOpts::with_reset_connection`]).
    ///
    ...
    /// Sets the given pening result metadata for this connection. Returns the
    /// previous value.
    ...
    /// Setup _local_ `LOCAL INFILE` handler (see ["LOCAL INFILE Handlers"][2]
    /// section of the crate-level docs).
    ///
    ...
    /// Immediately discards this connection without protocol cleanup.
    ///
    /// This does not drain pending results, roll back a transaction, or send
    /// `COM_QUIT`. Use it only when an interrupted operation may have left the
    /// protocol stream unusable.
    pub fn force_disconnect(mut self) {
        self.inner.disconnected = true;
        let pool = self.inner.pool.take();
        drop(self.inner.stream.take());
        if let Some(pool) = pool {
            pool.discard_in_use_connection();
        }
    }
    ...
    async fn handle_handshake(&mut self) -> Result<()> {
        ...
        // If we have a MariaDB server version, we are using mariadb extended
        // capabilities from the handshake packet. MariaDB does not set the
        // first standard capability flag bit to indicate that it supports extended
        // capabilities.
    ...
    async fn continue_parsec_auth(&mut self) -> Result<()> {
        let packet = self.read_packet().await?;
        // Noramally we need to skip escaping 0x01 byte. But in first parsec
        // implementations, server did not send it.
        let mut payload: &[u8] = &packet;
        if packet.first() == Some(&0x01) {
            payload = &packet[1..];
        }
        // At this point in future, when it will be possible for parsec to be default
        // authentication method, we can have authentication switch request. The
        // other possibele option here(and for now the only option) -
        // ext-salt packet.
    ...
    fn handle_packet(&mut self, packet: &PooledBuf) -> Result<bool> {
        ...
        if let Ok(ok_packet) = ok_packet {
            ...
            // capabilities and so it will behave as if we have none. In particular, the
            // error packet will not contain a SQL State field even if our
            // capabilities do contain the `CLIENT_PROTOCOL_41` flag. Therefore
            // it is necessary to parse an incoming packet with no capability
            // assumptions if we have not completed the handshake.
            //
    ...
    /// Will try to reconnect via socket using socket address in
    /// `self.inner.socket`.
    ///
    /// Won't try to reconnect if socket connection is already enforced in
    /// [`Opts`].
    ...
    ///
    /// * It reads and stores socket address inside the connection unless if
    ///   socket address is already in [`Opts`] or if `prefer_socket` is
    ///   `false`.
    ///
    /// * It reads and stores `max_allowed_packet` in the connection unless it's
    ///   already in [`Opts`]
    ///
    /// * It reads and stores `wait_timeout` in the connection unless it's
    ///   already in [`Opts`]
    ...
    ///
    /// Returns `false` if command is not supported (requires MySql >5.7.2,
    /// MariaDb >10.2.3). For older versions consider using
    /// [`Conn::change_user`].
    ///
    ...
    ///
    /// This might be used as an older and slower alternative to
    /// `COM_RESET_CONNECTION` that works on MySql prior to 5.7.3 (MariaDb
    /// prior ot 10.2.4).
    ///
    /// ## Note
    ///
    ...
    ///
    /// Will invoke `COM_CHANGE_USER` if `COM_RESET_CONNECTION` is not
    /// supported.
    ...
    pub(crate) async fn drop_result(&mut self) -> Result<()> {
        ...
        let meta = match self.set_pending_result(None)? {
            ...
            Some(PendingResult::Taken(meta)) => {
                // This also asserts that there is only one reference left to the taken
                // ResultSetMeta, therefore this result set must be dropped here
                // since it won't be dropped anywhere else.
                Some(Arc::try_unwrap(meta).expect(
                    "Conn::drop_result call on a pending result that may still be droped by \
                     someone else",
                ))
            },
            None => None,
    ...
    /// This function will drop pending result and rollback a transaction, if
    /// needed.
    ///
    /// The purpose of this function, is to cleanup the connection while
    ...
    async fn cleanup_for_pool(mut self) -> Result<Self> {
        loop {
            if self.has_pending_result() {
                ...
                // so we'll ignore non-fatal errors during cleanup (also there is no direct
                // caller to return this error to).
...
mod test {
    ...
    #[test]
    fn physical_connections_have_distinct_local_generations() {
        let first = Conn::empty(crate::Opts::default());
        let second = Conn::empty(crate::Opts::default());
        assert_ne!(first.local_generation(), second.local_generation());
    }

    #[tokio::test]
    async fn force_disconnect_drops_transport_and_releases_pool_slot() {
        ...
        pool.register_in_use_connection_for_test();
        let mut conn = Conn::empty(crate::Opts::default());
        conn.inner.stream = Some(stream);
        conn.inner.pool = Some(pool.clone());
        assert_eq!(pool.connection_counts_for_test(), (1, 1, 0));

        conn.force_disconnect();

        assert_eq!(pool.connection_counts_for_test(), (0, 0, 0));
        let mut byte = [0; 1];
        let bytes_read = timeout(Duration::from_secs(1), peer.read(&mut byte))
            .await
            .expect("force disconnect did not promptly close the transport")
            .unwrap();
    ...
    async fn should_connect() -> super::Result<()> {
        ...
        // Should connect with any combination of supported plugin and empty-nonempty
        // password.
        let variants = vec![
    ...
    async fn should_change_user() -> super::Result<()> {
        /// Whether particular authentication plugin should be tested on the
        /// current database.
        ...
        const TEST_MATRIX: [(&str, ShouldRunFn, CreateUserFn); 5] = [
            ...
            (
                ...
                |is_mariadb, version, pass| {
                    if is_mariadb {
                        vec![format!(
                            "CREATE USER '__mats'@'%' IDENTIFIED WITH mysql_native_password AS \
                             PASSWORD('{pass}')"
                        )]
                        ...
                        vec![format!(
                            "CREATE USER '__mats'@'%' IDENTIFIED WITH mysql_native_password BY \
                             '{pass}'"
                        )]
                    }
                },
        ...
        for (i, (plugin, should_run, create_statements)) in TEST_MATRIX.iter().enumerate() {
            ...
            if should_run(is_mariadb, version) {
                ...
                if matches!(version, (5, 6, _)) && i == 0 {
                    // IF EXISTS is not supported on 5.6 so the query will fail on the first
                    // iteration
                    drop(result);
                } else {
                    result.unwrap();
    ...
    async fn test_exec_batch() {
        ...
        conn.query_drop(
            "CREATE TEMPORARY TABLE t_exec_batch (id INT NOT NULL PRIMARY KEY,val VARCHAR(32),num \
             BIGINT UNSIGNED)",
        )
        .await
        .unwrap();

        // Populating table with some data to verify that data still fetched correctly
        // with cached metadata use
    ...
    async fn test_exec_batch_large() {
        ...
        // Calculate a row size that will make the total packet size >
        // max_allowed_packet Packet will have 4 byte header and 7 bytes
        // COM_STMT_BULK_EXECUTE fields + 4 bytes for parameter types
        // 8 bytes per row for id + 2 bytes for indicators + 3 bytes for length encoding
        // of val.
        let num_rows = 3;
        ...
        // MySql compression does not respect max_allowed_packet, so we're going to
        // query one by one
    ...
    async fn should_handle_initial_error_packet() {
        ...
        let error_message = "Host '172.17.0.1' is blocked because of many connection errors; \
